...
CREATE TABLE Militar_comandado(
    ...
    CONSTRAINT militar_comandado_pkey PRIMARY KEY (militar_comandante_cpf,militar_comandado_cpf),
    CONSTRAINT militar_comandado_fkey FOREIGN KEY (militar_comandado_fk, militar_comandante_cpf) REFERENCES militar (cpf),
    CONSTRAINT militar_comandado_fkey FOREIGN KEY (quadrante_coordenadas) REFERENCES Quadrante(coordenadas)
);

CREATE TABLE Visitante(
    nome VARCHAR2(100) NOT NULL,
    cpf_militar VARCHAR2(11) NOT NULL,
    CONSTRAINT Visitante_pkey PRIMARY KEY (nome,cpf_militar),
    CONSTRAINT visitante_fkey FOREIGN KEY (cpf_militar) REFERENCES militar (CPF)
);
...
CREATE TABLE Comunicação_entre_bases(
    ...
    CONSTRAINT Comunicação_entre_bases_fkey FOREIGN KEY (base_inicia_cominicação) REFERENCES Base_militar(nome),
    CONSTRAINT Comunicação_entre_bases_fkey FOREIGN KEY (base_recebe_comunicação) REFERENCES Base_militar(nome)
);
